...
class BatchUpdateMeanScore
  def self.calculate_data(year_data)
    ...
    count = 0.0
    decay = 0.3
    sums = {A:0.0, B:0.0, C:0.0, D:0.0, F:0.0, other:0.0, mean_score:0.0}
    ...
    faculties.each do |faculty|
      ...
      subjects.each do |subject|
        ...
        if subject_score = SubjectScore.find_by(subject_id: subject.id)
          subject_score.update(subject_id: subject.id, A: ret[:A], B: ret[:B], C: ret[:C], D: ret[:D], F: ret[:F], other: ret[:other], mean_score: ret[:mean_score], weighted_score: ret[:weighted_score])
        else
          SubjectScore.create(subject_id: subject.id, A: ret[:A], B: ret[:B], C: ret[:C], D: ret[:D], F: ret[:F], other: ret[:other], mean_score: ret[:mean_score], weighted_score: ret[:weighted_score])
        end
      end
    end
